  ...
  function generateProjects(data) {
    const projectsHTML = data.reduce((html, project) => {
      const figmaHTML = project.figmaLink
        ? `<a href=${project.figmaLink} aria-label="View the design on Figma" target="_blank" rel="noopener noreferrer"><img src="../images/figma.svg"
      alt="figma logo" /></a>`
        : "";

      return (html += `<div class="projects__project" tabindex=0>
      <div class="card">
        <a href=${project.link} aria-label=${project.label} target="_blank" rel="noopener noreferrer">
          <h3 class="card__title">${project.title}</h3>
          <p class="card__description">${project.description}</p>
        </a>
        <div class="card__links">
          <a href=${project.githubLink} aria-label="View the code on github" target="_blank" rel="noopener noreferrer"><svg width="1024" height="1024" viewBox="0 0 1024 1024" fill="none"
